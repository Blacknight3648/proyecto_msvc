package com.msvc.productos.exceptions;

import com.msvc.productos.dtos.ErrorDTO;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDTO> handleMedicoException(ProductoException exception){

        if(exception.getMessage().contains("no se encuentra en la base de datos")) {
            Map<String, String> errorMap = Collections.singletonMap("Atencion no encontrada", exception.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(this.createErrorDTO(HttpStatus.NOT_FOUND.value(), new Date(), errorMap));


        }else{
            Map<String, String> errorMap = Collections.singletonMap("Atencion existente", exception.getMessage());
