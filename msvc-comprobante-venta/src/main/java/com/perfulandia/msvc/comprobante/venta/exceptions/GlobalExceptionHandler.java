...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDTO> handleComprobanteException(ComprobanteException exception){
        if(exception.getMessage().contains("no se encuentra en la base de datos")){
            Map<String, String> errorMap = Collections.singletonMap("Comprobante no encontrado", exception.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(this.createErrorDTO(HttpStatus.NOT_FOUND.value(),new Date(),errorMap));
        }else {
            Map<String, String> errorMap = Collections.singletonMap("Comprobante existente", exception.getMessage());
